...
def add_to_database (parameters, noteNumber, data, orchestra):
    ...
    #nested_update(orchestra, {instrument:{tech:{dyn:{noteNumber:{"data":data, "fft":spectrum, "masking_curve":masking_threshold, "masking_locs":masking_freq, "lpc_curve":LpcFreqs, "lpc_locs":LpcLocs, "mfcc":mfcc_data, "centroid":centroid}}}}})
    nested_update(orchestra, {instrument: {tech: {dyn: {noteNumber: {"data": data}}}}})
    return orchestra
...
with open('orchestra_mini.pickle', 'wb') as f:
    # Pickle the 'data' dictionary using the highest protocol available.
    pickle.dump(orchestra, f, pickle.HIGHEST_PROTOCOL)